...
inline bool BeginWith_I(const char *s1, const char *s2)
{
	bool ok = true;
	for (; *s1 && *s2; ++ s1, ++ s2) {
...
struct SfPattern
{
	...
	bool Match(const char *s) {
		...
		if (matchBegin || matchEnd) {
			return BeginWith_I((matchBegin? s: s+len1-len2), pat);
		}
		else {
			for (; *s; ++s) {
				if (toupper(*s) == toupper(*pat) && BeginWith_I(s+1, pat+1))
					return true;
			}
		}
		return false;
...
char *GetFolderName(char *buf, const char *root, const char *dirname, const char *file = NULL, const char *line = NULL)
{
// 	static char buf[1024];
	// 1. replace root
...
void QueryFile(char *arg)
{
	...
	while (p = strtok(arg, " ")) {
		...
		bool matchBegin =false, matchEnd = false;
		if (*p1 == '/' || *p1 == '\\') {
			*p1 -- = 0;
			ppats = &pats_dir;
		}
		...
		if (p)
			ppats->push_back(SfPattern{p, matchBegin, matchEnd});
	}
	...
	while (it.Next()) {
		...
		if (ok) {
			...
			int cnt = items.size();
			char buf[1024];
			printf("%d:\t%s\t%s\n", cnt, file->name, GetFolderName(buf, repo->root, folder->name));
			if (cnt >= MAX_FOUND) {
				printf("... (max %d)\n", MAX_FOUND);
				break;
			}
	...
	while (it.Next()) {
		...
		if (ok) {
			...
			if (!g_forsvr)
				...
				printf("%d:\t%s\t%s\t%s\t%s:%s\t%s\n", cnt, kind, symbol->name(), symbol->extra(), file->name, symbol->line(), GetFolderName(buf, repo->root, folder->name));
			if (cnt >= MAX_FOUND) {
				printf("... (max %d)\n", MAX_FOUND);
				break;
			}
...
void GotoResult(const char *cmd, const char *arg)
{
	...
	if (idx >= 0 && idx < g_result.items.size()) {
		...
		GetFolderName(p, itm.repo->root, itm.folder->name, itm.file->name, line);
		printf("go %d: %s\n", idx+1, buf);
		system(buf);
	}
}
